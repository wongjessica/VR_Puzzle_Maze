...
public class Rotate : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Update(){
        if(!LockControl.isOpened && gazedAt){
            RaycastHit hit;
            int layerMask = 1 << 13;
...
/**
if(hit.collider.gameObject.name == "WheelOne"){
                    if(coroutineAllowed) StartCoroutine("RotateWheel");
                }
                else if(hit.collider.gameObject.name == "WheelTwo"){
                    if(coroutineAllowed) StartCoroutine("RotateWheel");
                }
                if(hit.collider.gameObject.name == "WheelThree"){
                    if(coroutineAllowed) StartCoroutine("RotateWheel");
                }
